...
String inputString = "";
bool stringComplete = false, isConnected = false;
char buf[60];
int i,j,k;
int diff = 5;
...
unsigned long timeNow = 0, timeInterval = 40, timeMemory = 0;
...
void loop() {
  ...
  timeNow = millis();
  if(timeNow - timeMemory >= timeInterval){
    kalmanFilter();
    regulator();
    timeMemory = timeNow;
  ...
  if (stringComplete) {
    ...
    serwo1.writeMicroseconds(map(fi1, 0, 45, 1350, 850));
